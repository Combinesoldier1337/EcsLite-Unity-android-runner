...
namespace Leopotam.EcsLite
{
    ...
    public interface IEcsFixedRunSystems : IEcsSystem
    {
    ...
    public class EcsSystems : IEcsSystems
    {
        ...
        readonly List<IEcsFixedRunSystems> _fixedRunSystems;
        ...
        public EcsSystems(EcsWorld defaultWorld, object shared = null)
        {
            ...
            _fixedRunSystems = new List<IEcsFixedRunSystems>(128);
        }
        ...
        public virtual IEcsSystems Add(IEcsSystem system)
        {
            ...
            else if (system is IEcsFixedRunSystems fixedRunSystems)
            {
                _fixedRunSystems.Add(fixedRunSystems);
            }
            return this;
        }
        ...
        {
#if DEBUG && !LEOECSLITE_NO_SANITIZE_CHECKS
            if (!_inited) { throw new System.Exception("Cant run without initialization. (Fixed Update)"); }
#endif
            for (int i = 0, iMax = _fixedRunSystems.Count; i < iMax; i++)
            {
                _fixedRunSystems[i].FixedRun(this);
#if DEBUG && !LEOECSLITE_NO_SANITIZE_CHECKS
                var worldName = CheckForLeakedEntities(this);
                if (worldName != null) { throw new System.Exception($"Empty entity detected in world \"{worldName}\" after {_runSystems[i].GetType().Name}.Run(). (Fixed Update)"); }
#endif
            }
        }
